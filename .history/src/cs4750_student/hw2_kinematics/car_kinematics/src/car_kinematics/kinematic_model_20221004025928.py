...
class KinematicCarMotionModel:
    ...
    def apply_motion_model(self, states, vel, alpha, dt):
        ...
        noisyvel = np.random.normal(vel, self.vel_std, n_particles).T
        noisyalp = np.random.normal(alpha, self.alpha_std, n_particles).T
        newcontrol = np.concatenate((noisyvel, noisyalp), axis=1)
        states = states+self.compute_changes(states, newcontrol, dt)
        return
