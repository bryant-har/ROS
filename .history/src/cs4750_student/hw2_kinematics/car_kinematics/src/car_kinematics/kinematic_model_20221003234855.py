...
class KinematicCarMotionModel:
    ...
    def compute_changes(self, states, controls, dt, alpha_threshold=1e-2):
        ...
        res[zeros, 0] = controls[:, 0] * dt*np.cos(states[:, 2])
        res[zeros, 1] = controls[:, 0] * dt*np.sin(states[:, 2])

        if nonzeros.size != 0:
            res[nonzeros, 2] = (controls[:, 0]/self.car_length) * \
                dt*np.tan(controls[:, 1])
            res[nonzeros, 0] = ((self.car_length)/np.tan(controls[:, 1])) * \
                (np.sin(states[:, 2]+res[:, 2])-np.sin(states[:, 2]))
            res[nonzeros, 1] = (((self.car_length)/np.tan(controls[:, 1]))
